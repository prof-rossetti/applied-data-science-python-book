...
The `pandas` package provides two new super-charged datatypes of interest: the `DataFrame` and the `Series`.

  + The [`DataFrame` datatype](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.html) represents the entire tabular data structure, a "matrix" of rows and columns.

  + The [`Series` datatype](https://pandas.pydata.org/docs/reference/api/pandas.Series.html) represents either a single row, or a single column in the dataset.
